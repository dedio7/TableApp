package com.example.tabletapp.calendar
...
fun rememberCalendarEvents(): CalendarState {
    ...
    LaunchedEffect(hasPermission) {
        if (hasPermission) {
            state = CalendarState(events = fetchCalendarEvents(context), hasPermission = true)
        } else {
...
private fun fetchCalendarEvents(context: Context): List<CalendarEvent> {
    ...
    return if (result.isEmpty()) {
        listOf(CalendarEvent("Nessun impegno", "Oggi", androidx.compose.ui.graphics.Color.Gray))
    } else {
        result.take(4)
    }
}
